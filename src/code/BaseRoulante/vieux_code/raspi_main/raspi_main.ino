...
struct move_info angular_move(float angle) {
  if (angle > 0.0) {
    dirL=-1;
    dirR=1;
  }
  ...
  // on transforme la distance en nombre de pulsation
  target = angle2encodervalue(angle);
  // on set la vitesse 
  speed_left = dirL*speed;
  speed_right = dirR*(COEFR2L*speed + BIASR2L);
...
void loop(){  
  ...
  front_col=move[2];
  coter_col=move[3];
  move_info angular_result = angular_move(move[0]);
  actionfaite[0] = angular_result.completion_action;
  if (angular_result.type_erreur == 0){
    move_info linear_result = linear_move(move[1]);
    actionfaite[1] = linear_result.completion_action;
    actionfaite[2] = linear_result.type_erreur;
  }
  else {
    actionfaite[1] = 0.0;
    actionfaite[2] = angular_result.type_erreur;
  }
